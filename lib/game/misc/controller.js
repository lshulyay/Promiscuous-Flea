...
ig.Controller = ig.Class.extend({
	...
	checkDetection: function(entity) {
		var primaryEye = ig.game.primaryEye;
		var dist = Math.sqrt(Math.pow(primaryEye.pointOfSight.x - entity.pos.x,2)+Math.pow(primaryEye.pointOfSight.y - entity.pos.y,2));
		if (dist<(primaryEye.sightArea + entity.size.x + entity.size.x / 2)) return true;
    },
	...
	setRandomLocation: function() {
		...
		if (spawnX >= 150 && spawnX <= 360 &&
			...
			console.log('uh oh in eye 1');
			return this.setRandomLocation();
		}
		else if (spawnX >= 522 && spawnX <= 735 &&
				spawnY >= 220 && spawnY <= 433) {
			console.log('uh oh in eye 2');
			return this.setRandomLocation();
		}

		else {
			console.log('spawn point OK: ' + spawnX + ' ' + spawnY);
