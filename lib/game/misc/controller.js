...
ig.Controller = ig.Class.extend({
	...
	calcScore: function(score) {
		...
		this.score += score;
		if (this.score > oldScore && this.score%5 === 0) {
			this.difficulty += 0.5;
			ig.game.primaryEye.newSightArea = ig.game.primaryEye.defaultSightArea * this.difficulty;
			if (this.minLifeTime >= 15) {
				this.minLifeTime -= 5;
				this.maxLifeTime -= 5;
			}
			if (ig.game.eyeGuide.lookTime >= 1.5) {
				ig.game.eyeGuide.lookTime -= 0.5;
				ig.game.eyeGuide.lookTimer.set(ig.game.eyeGuide.lookTime);
			}
		}
	},
